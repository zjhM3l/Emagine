...
public class TradeAdapter extends ArrayAdapter {
    public TradeAdapter(@NonNull Context context, int resource, @NonNull List<Want> objects) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        ImageView reqimage =view.findViewById(R.id.want_other_image);

        String username = want.getReqName();
        LoginUser userData = LitePal.where("name = ?", username).findFirst(LoginUser.class);

        if (userData.getPortrait() != null) {
            reqimage.setImageBitmap(Bytes2Bimap(userData.getPortrait()));
        } else {
            reqimage.setImageResource(R.drawable.user_image);
        }


        artimage.setImageBitmap(Bytes2Bimap(want.getImage()));
        traderole.setText("WANT");
